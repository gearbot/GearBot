    ...
    try:
        ...
        if startup:
            ...
            try:
                ...
                if socket == "":
                    bot.redis_pool = await aioredis.create_redis_pool((Configuration.get_master_var('REDIS_HOST',
                                                                                                    "localhost"),
                                                                       Configuration.get_master_var('REDIS_PORT',
                                                                                                    6379)),
                                                                      encoding="utf-8", db=0)
                else:
        ...
        if isinstance(ctx.channel, TextChannel) and not ctx.channel.permissions_for(ctx.channel.guild.me).send_messages:
            try:
                await ctx.author.send(
                    "Hey, you tried triggering a command in a channel I'm not allowed to send messages in. Please grant me permissions to reply and try again.")
            except Forbidden:
                pass  # closed DMs
        elif ctx.author.id in Configuration.get_persistent_var("user_blacklist", []):
            try:
                await MessageUtils.send_to(ctx, "BAD_USER",
                                           "You have been globally blacklisted from using this bot due to abuse",
                                           translate=False)
            except Forbidden:
                pass  # closed DMs
        else:
...
async def on_guild_join(guild):
    ...
    if guild.id in blocked:
        ...
        GearbotLogging.info(
            f"Someone tried to add me to {guild.name} ({guild.id}) but the owner ({guild.owner} ({guild.owner.id})) is blacklisted")
        try:
            await guild.owner.send(
                f"Someone tried adding me to {guild.name} (``{guild.id}``) but you have been blacklisted due to bot abuse, so i left")
        except Exception:
            pass
        await guild.leave()
    else:
        ...
        await GearbotLogging.bot_log(f"{Emoji.get_chat_emoji('JOIN')} A new guild came up: {name} ({guild.id}).",
...
async def on_command_error(bot, ctx: commands.Context, error):
    if isinstance(error, commands.BotMissingPermissions):
        ...
        if ctx.command.qualified_name is not "latest" and ctx.guild is not None and Configuration.get_var(ctx.guild.id,
                                                                                                          "GENERAL",
                                                                                                          "PERM_DENIED_MESSAGE"):
        ...
        bot.help_command.context = ctx
        await ctx.send(
            f"{Emoji.get_chat_emoji('NO')} {Translator.translate('bad_argument', ctx, type=error.type, error=error.error)}\n{Emoji.get_chat_emoji('WRENCH')} {Translator.translate('command_usage', ctx, usage=bot.help_command.get_command_signature(ctx.command))}")
    ...
    # TODO: rework this handling later

    else:
        await handle_exception("Command execution failed", bot, error.original if hasattr(error, "original") else error,
                               ctx=ctx)
        # notify caller
...
#     database trouble, notify bot owner
    ...
    #     fail, trying again in 10 just in case the database is rebooting
        ...
        #             await session.post(
        #                 f'https://api.digitalocean.com/v2/droplets/{Configuration.get_master_var("DO_ID")}/actions',
