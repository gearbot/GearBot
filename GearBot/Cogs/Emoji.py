...
class Emoji(BaseCog):
    ...
    async def cog_check (self, ctx):
        return hasattr(ctx.author, '_roles') and await Permissioncheckers.check_permission(ctx.command, ctx.guild, ctx.author, self.bot) or ctx.channel.permissions_for(ctx.author).manage_emojis
