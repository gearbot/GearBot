...
class GlobalInfSearch(View):
    def __init__(self, filters, pages, guild_id, current_page=0):
        super().__init__(timeout=None)
        self.id=""
        if pages > 0:
            self.add_item(CallbackButton(Translator.translate('first_page', guild_id), self.on_first_page, 'ginf_search:first_page', disabled=current_page == 0))
            self.add_item(CallbackButton(Translator.translate('prev_page', guild_id), self.on_prev_page, 'ginf_search:prev_page', disabled=current_page == 0))
            self.add_item(CallbackButton(emoji=Emoji.get_emoji('AE'), style=ButtonStyle.grey, cid='ginf_search:blank', callback=self.hi, label=None))
            self.add_item(CallbackButton(Translator.translate('next_page', guild_id), self.on_next_page, 'ginf_search:next_page', disabled=current_page >= pages-1))
            self.add_item(CallbackButton(Translator.translate('last_page', guild_id), self.on_last_page, 'ginf_search:last_page', disabled=current_page >= pages-1))
    ...
    async def on_first_page(interaction: Interaction):
        page, current, pages, query, fields = await get_global_cached_page(interaction, 100)
        await interaction.response.edit_message(
            content=await InfractionUtils.assemble_message(interaction.guild_id, page, query, current, pages, globaly=True),
            view=GlobalInfSearch(filters=fields, pages=pages, current_page=current, guild_id=interaction.guild_id)
        )


    @staticmethod
    async def on_prev_page(interaction: Interaction):
        page, current, pages, query, fields = await get_global_cached_page(interaction, -1)
        await interaction.response.edit_message(
            content=await InfractionUtils.assemble_message(interaction.guild_id, page, query, current, pages, globaly=True),
            view=GlobalInfSearch(filters=fields, pages=pages, current_page=current, guild_id=interaction.guild_id)
        )
...
async def get_global_cached_page(interaction: Interaction, diff):
