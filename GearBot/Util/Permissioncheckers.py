...
async def check_permission(command_object, guild, member, bot):
    if not hasattr(member, '_roles') and guild is not None:
        member = await Utils.get_member(bot, guild, member.id)
    if guild is None:
