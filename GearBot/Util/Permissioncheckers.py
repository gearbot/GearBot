...
def get_required(ctx, perm_dict):
    return get_perm_dict(ctx.bot.get_command(ctx.message.content[len(ctx.prefix):]).qualified_name.split(" "), perm_dict)["required"]
...
def get_user_lvl(ctx:commands.Context):
    ...
    if cog_name in overrides:
        ...
        pieces = ctx.bot.get_command(ctx.message.content[len(ctx.prefix):]).qualified_name.lower().split(" ")
