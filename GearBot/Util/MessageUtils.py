...
from database.Models import LoggedMessage
...
async def get_message_data(bot, message_id):
    message = None
    if not Object(message_id).created_at <= datetime.utcfromtimestamp(time.time() - 5 * 60):
        parts = await bot.redis_pool.hgetall(f"messages:{message_id}")
        if len(parts) is 6:
            message = Message(message_id, int(parts["author"]), parts["content"], int(parts["channel"]),
                              int(parts["server"]), [attachment(a.split("/")[0], a.split("/")[1]) for a in
                                                     parts["attachments"].split("|")] if len(
                    parts["attachments"]) > 0 else [], type=int(parts["type"]) if "type" in parts else None,
                              pinned=parts["pinned"] == '1')
    if message is None:
        message = LoggedMessage.get(id=message_id)
    return message
...
async def insert_message(bot, message, redis=True):
    ...
    if redis:
        ...
        pipe.hmset_dict(f"messages:{message.id}", author=message.author.id, content=message.content,
                        channel=message.channel.id, server=message.guild.id, pinned=1 if message.pinned else 0,
                        attachments='|'.join((f"{str(a.id)}/{str(a.filename)}" for a in message.attachments)))
...
async def update_message(bot, message_id, content, pinned):
    if not Object(message_id).created_at <= datetime.utcfromtimestamp(time.time() - 5 * 60):
        ...
        await pipe.execute()
    await LoggedMessage.filter(id=message_id).update(content=content, pinned=pinned)
